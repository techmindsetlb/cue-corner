...
4. Your live URLs will be:
   ...
   - **Admin**: `https://YOUR_USERNAME.github.io/cafe-menu/backend/`
...
2. **Rename the admin folder** — before pushing to GitHub, rename `backend/` to something random. Update the link in `index.html`'s sidebar. This obscures the admin URL.
...
Put the entire `/backend/` path behind Cloudflare Access — it prompts for a Google/email login before the page even loads.
